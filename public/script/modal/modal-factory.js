...
const create = () => {
    const openers = Array.from(document.querySelectorAll('.js-modal-opener'));
    const backgroundElement = document.querySelector('.js-modal-background');
    ...
    const modalMediators = openers.map(opener => {
        ...
        const overlay = content?.querySelector('.js-overlay');
        const focusableElements = content?.querySelectorAll(focusableElementsQuery) ?? [];
