 ...
 *
 */
const modalMediator = ({ opener, content, overlay, backgroundElement, focusableElements }: ModalMediatorParam) => {
  ...
  const open = (event: MouseEvent | KeyboardEvent) => {
    ...
    // モーダルを開いた時のフォーカス制御
    content?.focus()
    const firstFocusElement = content?.querySelector<HTMLElement>('[data-focus-primary]')
  ...
  const setCloserListeners = (shouldSet: Boolean) => {
    closers.forEach(closer => {
      closer.removeEventListener('click', onClose)
      if (shouldSet) closer.addEventListener('click', onClose, { once: true })
    })
  }
   ...
   * @param shouldSet
   */
  const setOpenerListeners = (shouldSet: Boolean) => {
    opener.removeEventListener('click', open)
    if (shouldSet) opener.addEventListener('click', open, { once: true })
  }
