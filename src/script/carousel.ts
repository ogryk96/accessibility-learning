...
const createCarousel = (options: carouselParams) => {
  ...
  const onControlButtonClick = (event: Event) => {
  ...
  //- フォーカス制御
  const onFocusInAndOut = (event: FocusEvent) => {
  ...
  const onMouseOut = (event: MouseEvent) => {
    if (!ignoreHoverEvents) {
      ...
      const isFocusableElementFocused =
        document.activeElement && document.activeElement.closest('.carousel') === carousel
