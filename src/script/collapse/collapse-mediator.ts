interface CollapseMediatorParams {
  switcher: HTMLElement
  content: HTMLElement
  index: number
}

type CollapseStatus = 'closed' | 'open' | 'closing' | 'opening'
...
const collapseMediator = ({ switcher, content, index }: CollapseMediatorParams) => {
   ...
   * ARIA 属性を更新
   */
  const updateAriaAttrs = (isOpen: boolean) => {
  ...
  const emitEvent = () => {
    ...
    const event = new CustomEvent<CollapseEvent>(eventType, eventOptions)
    // console.log(event)
    dispatchEvent(event)
  }
